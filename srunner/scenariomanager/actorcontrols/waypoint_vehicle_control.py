...
class WaypointVehicleControl(BasicControl):
    ...
    def _set_new_velocity(self, next_location):
        ...
        target_speed = self._get_target_speed(current_time)
    ...
    def _get_target_speed(self, current_time):
        tmp_time = int(current_time)
        print(current_time)
        speed = 0
        if tmp_time > len(self.speed_profile) :
            speed = self.speed_profile[-1]
        else:
            speed = self.speed_profile[tmp_time-1]
